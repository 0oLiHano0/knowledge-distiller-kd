...
from kd_tool.stages.decision.errors import (
    DecisionError,
    RuleEvaluationError,
    MissingAnalysisDataError,
)
...
class DecisionStage(StageInterface):
    ...
    def __init__(self, logger: LoggerProtocol, settings: DecisionStageSettings):
        ...
        self._sorted_rules = sorted(
            self._settings.rules, key=lambda r: r.rule_priority, reverse=True
        )
            ...
            for (b1_id, b2_id), results_map in unique_pairs.items():
                try:
                    ...
                    if (
                        ...
                        or decision_type_to_apply == DecisionType.UNDECIDED
                        and self._settings.process_undecided
                        or decision_type_to_apply != DecisionType.UNDECIDED
                        and decision_type_to_apply != DecisionType.KEEP
                    ):
                        representative_result = self._get_representative_result(
                            results_map
                        )
    ...
    def _get_unique_pairs_with_results(
    ...
    def _get_representative_result(
        self, results_map: Dict[AnalysisType, AnalysisResultDTO]
    ...
    def _apply_rules(
        self, results_map: Dict[AnalysisType, AnalysisResultDTO], logger: LoggerProtocol
    ) -> DecisionType:
    ...
    def _check_rule_match(
        self, rule: DecisionRule, results_map: Dict[AnalysisType, AnalysisResultDTO]
    ) -> bool:
        if rule.md5_score is not None:
            md5_result = results_map.get(AnalysisType.MD5)
            if (
                md5_result is None
                or md5_result.score is None
                or md5_result.score != rule.md5_score
            ):
                return False
        if rule.simhash_similarity_min is not None:
            simhash_result = results_map.get(AnalysisType.SIMHASH)
            if (
                simhash_result is None
                or simhash_result.score is None
                or simhash_result.score < rule.simhash_similarity_min
            ):
                return False
        if rule.semantic_similarity_min is not None:
            ...
            if (
                ...
                or semantic_result.score is None
                or semantic_result.score < rule.semantic_similarity_min
            ):
                return False
        return True
