...
class DecisionRule(BaseModel):
    ...
    **规范**: 用于描述当满足某些分析条件时，应采取何种决策。
    ...
    md5_score: Optional[float] = Field(
        default=None,
        ge=0.0,
        le=1.0,
        description="触发此规则的 MD5 分数 (通常是 1.0)。如果为 None，则不考虑 MD5。",
    )
    simhash_similarity_min: Optional[float] = Field(
        default=None,
        ge=0.0,
        le=1.0,
        description="触发此规则的 SimHash 最小相似度。如果为 None，则不考虑。",
    )
    semantic_similarity_min: Optional[float] = Field(
        default=None,
        ge=0.0,
        le=1.0,
...
class DecisionStageSettings(BaseModel):
    ...
    rules: List[DecisionRule] = Field(
        default_factory=lambda: [
            DecisionRule(
                md5_score=1.0, decision_to_apply=DecisionType.DELETE, rule_priority=100
            ),
            DecisionRule(
