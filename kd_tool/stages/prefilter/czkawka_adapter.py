...
from kd_tool.stages.prefilter.dtos import (
    CzkawkaScanOutputDTO,
...
class CzkawkaAdapter(CzkawkaAdapterInterface):
    ...
    def scan_and_find_duplicates(self) -> CzkawkaScanOutputDTO:
        ...
        try:
            ...
            raise CzkawkaExecutionError(
                command=" ".join(command), return_code=-1, error_output=str(e)
            ) from e
        ...
        try:
            ...
            if "duplicates" not in data:
                raise CzkawkaParseError(
                    "JSON output missing 'duplicates' key.", json_output
                )
            duplicate_groups = [
                CzkawkaDuplicateResultDTO(
            ...
            return CzkawkaScanOutputDTO(
                all_scanned_files=all_files, duplicate_groups=duplicate_groups
            )
            ...
            raise CzkawkaParseError(
                f"Unexpected parsing error: {e}", json_output
            ) from e
