...
class PrefilterStage(StageInterface):
    ...
    def process(self, context: PipelineContextDTO) -> PipelineContextDTO:
        ...
        **[指令]** 严禁直接调用 `storage` 进行写入操作，**必须**通过 `PipelineContextDTO` 进行状态同步。
        """
        run_logger: LoggerProtocol = context.run_logger.bind(
        ...
        try:
            ...
            if self._settings.register_files_in_storage:
                ...
                file_dtos_to_register = self._prepare_scan_results(
                    scan_output, run_logger
                )
                run_logger.success(
                    f"扫描结果处理完成. 准备注册/更新 {len(file_dtos_to_register)} 个文件."
                )
            else:
                run_logger.warning("注册文件到存储已禁用.")
            if file_dtos_to_register:
                ...
                run_logger.success("PipelineContextDTO 更新完成.")
            else:
                run_logger.info(
            ...
            run_logger.error(f"预过滤阶段执行错误: {e}", exc_info=True)
            context.add_error(e)
        except Exception as e:
            run_logger.exception("预过滤阶段执行意外错误.")
            error = PrefilterError(f"预过滤执行失败: {e}", original_exception=e)
            context.add_error(error)
        run_logger.info("预过滤阶段完成.")
        return context

    def _prepare_scan_results(
        self, scan_output: CzkawkaScanOutputDTO, run_logger: LoggerProtocol
    ) -> List[FileRecordDTO]:
        """
        ...
        **[指令]** 调用 `self._storage.register_files` 时 **严禁** 传递 `task_id` (除非接口明确要求)。
        ...
        run_logger.debug(f"处理 {len(scan_output.duplicate_groups)} 个重复组...")
        for group in scan_output.duplicate_groups:
            ...
            if original_path not in files_to_register_map:
                files_to_register_map[original_path] = FileRecordDTO(
                    ...
                    processing_status=ProcessingStatus.PREPROCESSING_COMPLETED,
                    metadata={
        ...
        for file_path_obj in scan_output.all_scanned_files:
            ...
            if (
                file_path not in processed_paths_from_duplicates
                and file_path not in files_to_register_map
            ):
                try:
                    size = file_path.stat().st_size
                except OSError as e:
                    size = -1
                    run_logger.warning(f"无法获取文件大小: {file_path}. 错误: {e}")
        ...
        if file_dtos_list:
            run_logger.info(f"尝试注册/更新 {len(file_dtos_list)} 个文件到存储...")
            return file_dtos_list
        else:
            run_logger.warning("没有文件找到注册或更新到存储.")
        return []
