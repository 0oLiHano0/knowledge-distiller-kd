...
from kd_tool.stages.docprocessing.document_processing_stage import (
...
class DocumentProcessingStageFactory:
    ...
    def _build_parser_adapter(
        self, parser_type: str, parsing_strategy: str
    ) -> ParserAdapterInterface:
        """
        ...
        if parser_type == "unstructured":
            return UnstructuredParserAdapter(
                self._logger.bind(component="UnstructuredParserAdapter")
            )
    ...
    def create(self, settings: DocumentProcessingStageSettings) -> StageInterface:
        ...
        parser_adapter = self._build_parser_adapter(
            settings.parser_type, settings.parsing_strategy
        )
