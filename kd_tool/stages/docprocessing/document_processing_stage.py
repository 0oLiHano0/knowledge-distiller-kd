...
from kd_tool.stages.docprocessing.errors import (
...
class DocumentProcessingStage(StageInterface):
    ...
    def __init__(
        ...
        parser_adapter: ParserAdapterInterface,
    ) -> None:
    ...
    def process(self, context: PipelineContextDTO) -> PipelineContextDTO:
        ...
        files_to_process = [
            record
            for record in context.file_records.values()
            ...
            try:
                if file_path.suffix.lower() not in self._settings.supported_extensions:
                    raise UnsupportedFileTypeError(
                        file_path, detected_file_type=file_path.suffix
                    )
                raw_elements = self._parser.parse_file_to_raw_elements(file_path)
                if not raw_elements:
                    logger.warning(f"没有从 {file_path} 提取到原始元素. 文件处理为空.")
                    ...
                    file_record.processing_history.append(
                        {
                            ...
                            "details": "No raw elements extracted.",
                        }
                    )
                    continue
                ...
                file_record.metadata["p03_extracted_block_count"] = len(
                    preliminary_blocks
                )
                file_record.processing_history.append(
                    {
                        "stage": "P03",
                        "status": ProcessingStatus.BLOCK_EXTRACTION_COMPLETED.value,
                        "timestamp": datetime.datetime.now(
                            datetime.timezone.utc
                        ).isoformat(),
                        "details": f"Extracted {len(preliminary_blocks)} preliminary blocks.",
                    }
                )
                ...
                logger.exception(f"处理 {file_path} 时发生严重错误.")
        ...
        for index, raw_element in enumerate(raw_elements):
            ...
            try:
                ...
                if "Title" in raw_element_type_str or "Heading" in raw_element_type_str:
                    ...
                    "Code" in raw_element_type_str
                    or "CodeSnippet" in raw_element_type_str
                ):
