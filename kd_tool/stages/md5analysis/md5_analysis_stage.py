...
class MD5AnalysisStage(StageInterface):
    ...
    def process(self, context: PipelineContextDTO) ->PipelineContextDTO:
        ...
        if not content_blocks_to_check:
            ...
            return context
        logger.info(
            f'找到 {len(content_blocks_to_check)} 个块需要计算 MD5.')
        ...
        for block in content_blocks_to_check:
            try:
                ...
                logger.error(
                    f'MD5 处理错误: {block.block_id}: {e}')
                context.add_error(e)
                error_count += 1
            except Exception as e:
                logger.exception(
                    f'MD5 处理错误: {block.block_id}.'
                    )
        ...
        for md5_hash, block_ids in md5_map.items():
            if len(block_ids) > 1:
                ...
                logger.debug(
                    f'找到重复组 (MD5: {md5_hash}): {block_ids}')
