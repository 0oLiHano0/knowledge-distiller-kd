...
class SimHashAnalysisStageSettings(BaseModel):
    ...
    enabled: bool = Field(
        default=True, description="是否启用 P06 - SimHash 分析阶段 (用于近似去重)。"
    )
    ...
    force_recalculate: bool = Field(
        default=False, description="是否强制重新计算所有块的 SimHash 值。"
    )
    ...
    def hamming_distance_threshold(self) -> int:
        ...
        return max(
            0,
            min(
                self.hash_bits, int(ceil(self.hamming_distance_ratio * self.hash_bits))
            ),
        )
