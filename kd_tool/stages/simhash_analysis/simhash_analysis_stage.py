...
from kd_tool.stages.simhash_analysis.errors import (
    SimHashAnalysisError,
    SimHashCalculationError,
    SimHashComparisonError,
)
...
class SimHashAnalysisStage(StageInterface):
        ...
        **[指令]** 严禁直接调用 `storage` 进行写入操作，**必须**通过 `PipelineContextDTO` 进行状态同步。
        ...
        try:
            ...
            if not blocks_to_process:
                ...
                return context
            run_logger.info(
                f"找到 {len(blocks_to_process)} 个内容块需要进行 SimHash 处理。"
            )
            ...
            if blocks_with_hash:
                self._update_context_hashes(blocks_with_hash, run_logger, context)
            ...
            self._save_analysis_results_to_context(
                analysis_results, run_logger, context
            )
            run_logger.success("SimHash 分析阶段执行完毕。")
        except Exception as e:
            error = SimHashAnalysisError(
                f"SimHash 分析阶段发生未知错误: {e}", original_exception=e
            )
    ...
    def _get_blocks_to_process(
        ...
        if self._settings.force_recalculate:
            ...
            return [
                b for b in context.content_blocks.values() if b.simhash_value is None
            ]

    def _calculate_hashes(
        self, blocks: List[ContentBlockDTO], logger: LoggerProtocol
        ...
        for block in blocks:
            if not block.analysis_text:
                ...
                continue
            try:
                hash_value = self._adapter.calculate_simhash(
                    block.analysis_text, hash_bits
                )
    ...
    def _update_context_hashes(
        ...
        try:
            ...
            err = SimHashAnalysisError(
                f"更新 SimHash 值到 context 时失败: {e}", original_exception=e
            )
            logger.error(err)
            context.add_error(err)

    def _get_all_blocks_for_comparison(
        self, context: PipelineContextDTO
    ) -> List[ContentBlockDTO]:
        return [
            b for b in context.content_blocks.values() if b.simhash_value is not None
        ]

    def _compare_hashes_and_generate_results(
        self, blocks: List[ContentBlockDTO], logger: LoggerProtocol, task_id: str
        ...
        if len(blocks) < 2:
            return []
        block_hashes = [
            (b.block_id, b.simhash_value) for b in blocks if b.simhash_value
        ]
        if not block_hashes:
            return []
        ...
        try:
            if hasattr(self._adapter, "find_similar_pairs") and callable(
                self._adapter.find_similar_pairs
            ):
                logger.debug("使用适配器的 `find_similar_pairs` 方法进行比较。")
                similar_pairs = self._adapter.find_similar_pairs(
                    block_hashes, threshold, hash_bits
                )
            else:
                logger.warning(
                    "SimHash 适配器未提供 `find_similar_pairs`，将进行 O(n^2) 比较。"
                )
                similar_pairs = self._brute_force_comparison(
                    block_hashes, threshold, logger
                )
            ...
            err = SimHashComparisonError(
                f"比较 SimHash 时发生错误: {e}", original_exception=e
            )
            logger.error(err)
        return results

    def _brute_force_comparison(
        self,
    ...
    def _save_analysis_results_to_context(
        self,
