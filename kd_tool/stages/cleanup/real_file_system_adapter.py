...
class RealFileSystemAdapter(FileSystemAdapterInterface):
    ...
    def move_file(self, source_path: Path, target_path: Path) -> None:
        """移动文件。"""
        self._logger.debug(
            f"尝试移动文件从 '{source_path}' 到 '{target_path}'..."
        )  # 添加日志
        try:
            ...
            self.ensure_directory_exists(
                target_path.parent
            )  # ensure_directory_exists 内部会有自己的日志
            shutil.move(str(source_path), str(target_path))
            self._logger.info(
                f"成功移动文件从 '{source_path}' 到 '{target_path}'."
            )  # 成功日志
        except Exception as e:
            self._logger.exception(
                f"移动文件 '{source_path}' 到 '{target_path}' 时发生错误."
            )  # 异常日志
