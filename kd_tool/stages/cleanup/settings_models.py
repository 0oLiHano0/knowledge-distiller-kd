...
class CleanupStageSettings(BaseModel):
    ...
    **架构师说明**: 此阶段将执行 `DecisionStage` 产生的决策。
    ...
    action_map: Dict[
        DecisionType,
        Literal["mark_only", "move_to_trash", "permanent_delete", "ignore"],
    ] = Field(
        default_factory=lambda: {
            DecisionType.DELETE: "mark_only",
            DecisionType.KEEP: "ignore",
            DecisionType.UNDECIDED: "ignore",
            DecisionType.IGNORE_PAIR: "ignore",
        },
        description="""
        决策类型到具体清理动作的映射。
    ...
    def check_trash_dir_if_needed(cls, data: Any) -> Any:
        ...
        if isinstance(data, cls):
            needs_trash = any(
                action == "move_to_trash" for action in data.action_map.values()
            )
