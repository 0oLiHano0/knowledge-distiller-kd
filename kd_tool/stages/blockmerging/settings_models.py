...
class TextBlockMergeSettings(BaseModel):
    ...
    short_text_char_threshold: PositiveInt = Field(
        default=50, description="被视为空短文本块的字符数阈值，这类块更容易被合并。"
    )
    max_merged_text_block_length_char: PositiveInt = Field(
        default=2000, description="合并后文本块的最大允许字符长度。"
    )
...
class BlockMergerStageSettings(BaseModel):
    ...
    types_to_attempt_merge: List[BlockType] = Field(
        default=[BlockType.CODE_SNIPPET, BlockType.NARRATIVE_TEXT, BlockType.LIST_ITEM],
    ...
    min_block_length_char: PositiveInt = Field(
        default=100, description="合并后块的期望最小字符长度（启发式规则）。"
    )
    max_block_length_char: PositiveInt = Field(
        default=2000, description="合并后块的期望最大字符长度（启发式规则）。"
    )
    code_block_settings: CodeBlockMergeSettings = Field(
        default_factory=CodeBlockMergeSettings
    )
    text_block_settings: TextBlockMergeSettings = Field(
        default_factory=TextBlockMergeSettings
    )
