...
class SemanticAnalysisStageSettings(BaseModel):
    ...
    **规范**: 定义语义分析模型、阈值和执行参数。
    ...
    model_name_or_path: str = Field(default=
        'shibing624/text2vec-base-chinese', description=
        """
    ...
    batch_size: PositiveInt = Field(default=32, description=
        """
        向量嵌入批处理大小。
        **规范**: 影响性能和显存占用。适配器应支持按此批次大小处理文本。
        """
        )
    device: Optional[str] = Field(default=None, description=
        """
        运行模型设备 (e.g., 'cpu', 'cuda', 'cuda:0')。
        **规范**: 如果为 None，库通常会自动选择。适配器应能处理此参数。
        """
        )
    comparison_strategy: Literal['all_pairs', 'pre_filtered'] = Field(default
        ='pre_filtered', description=
        """
        比较策略。
