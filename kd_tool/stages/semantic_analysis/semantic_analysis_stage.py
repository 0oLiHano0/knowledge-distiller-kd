    ...
    def process(self, context: PipelineContextDTO) ->PipelineContextDTO:
        ...
        **[指令]** 调用 `storage.save_analysis_results` 时
        ...
        try:
            ...
            texts_to_embed = [b.analysis_text for b in blocks_to_process if
                b.analysis_text is not None and b.analysis_text.strip() != '']
            valid_blocks_for_embedding = [b for b in blocks_to_process if b
                .analysis_text is not None and b.analysis_text.strip() != '']
            valid_block_ids_for_embedding = [b.block_id for b in
            ...
            embeddings = self._adapter.calculate_embeddings(texts_to_embed,
                self._settings.batch_size)
            embedding_map = {block_id: emb for block_id, emb in zip(
                valid_block_ids_for_embedding, embeddings)}
        ...
        for block_id_current in block_ids:
            if block_id_current in context.analysis_results:
                for analysis_type in [AnalysisType.MD5, AnalysisType.SIMHASH]:
                    if analysis_type in context.analysis_results[
                        block_id_current]:
                        for result in context.analysis_results[block_id_current
                            ][analysis_type]:
                            if (result.analysis_type == AnalysisType.MD5 and
                                result.score == 1.0):
                                existing_matches_pair_ids.add(result.
                                    pair_analysis_id)
                            elif result.analysis_type == AnalysisType.SIMHASH and result.score is not None and result.score >= self._settings.similarity_threshold:
                                existing_matches_pair_ids.add(result.
                                    pair_analysis_id)
        for i in range(n):
            for j in range(i + 1, n):
                b1_id, b2_id = block_ids[i], block_ids[j]
                temp_pair_id_md5 = (AnalysisResultDTO.
                    _calculate_pair_analysis_id(b1_id, b2_id, AnalysisType.MD5)
                    )
                temp_pair_id_simhash = (AnalysisResultDTO.
                    _calculate_pair_analysis_id(b1_id, b2_id, AnalysisType.
                    SIMHASH))
            ...
            try:
                ...
                if similarity >= threshold:
                    result = AnalysisResultDTO(block_id_1=b1_id, block_id_2
                        =b2_id, analysis_type=AnalysisType.SEMANTIC, score=
                        similarity, details={'model_name': self._settings.
                        model_name_or_path, 'threshold': threshold})
