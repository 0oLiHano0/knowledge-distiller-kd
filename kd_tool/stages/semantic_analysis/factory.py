...
from kd_tool.stages.semantic_analysis.semantic_analysis_stage import (
    SemanticAnalysisStage,
)
from kd_tool.stages.semantic_analysis.adapter_interface import SemanticAdapterInterface
from kd_tool.stages.semantic_analysis.sentence_transformer_adapter import (
...
class SemanticAnalysisStageFactory:
    ...
    def create(
        ...
        **返回**:
        ...
        if adapter is None:
            self._logger.debug(
                "未提供语义分析适配器，创建默认的 SentenceTransformerAdapter..."
            )
