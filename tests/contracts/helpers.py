...
def compare_method_signatures(
    ...
    # 1. 比较参数数量
    assert len(interface_sig.parameters) == len(implementation_sig.parameters), \
        ...
        if interface_param.annotation != inspect.Parameter.empty:
            assert interface_param.annotation == impl_param.annotation, \
                (f"契约错误({class_name}.{method_name}): "
                 ...
                 f"接口:{interface_param.annotation}, 实现:{impl_param.annotation}.")

        # 2c. 比较默认值
        assert interface_param.default == impl_param.default, \
            (f"契约错误({class_name}.{method_name}): "
    ...
    if interface_sig.return_annotation != inspect.Signature.empty:
        assert interface_sig.return_annotation == implementation_sig.return_annotation, \
            (f"契约错误({class_name}.{method_name}): "
             f"返回类型注解不符。"
