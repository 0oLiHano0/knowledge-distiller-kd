...
def dummy_settings():
    ...
    what: 返回 MD5AnalysisStageSettings 实例
    how: 直接实例化
    """
    return MD5AnalysisStageSettings()

def test_init_signature(dummy_logger, dummy_settings):
    """
    为什么: 检查 MD5AnalysisStage 的 __init__ 是否支持依赖注入。
    做什么: 断言 __init__ 参数包含 logger 和 settings。
    怎么做: 用 inspect 获取签名。
    """
    sig = inspect.signature(MD5AnalysisStage.__init__)
...
def test_process_signature():
    ...
    怎么做: 用 inspect 获取签名。
    """
    sig = inspect.signature(MD5AnalysisStage.process)
