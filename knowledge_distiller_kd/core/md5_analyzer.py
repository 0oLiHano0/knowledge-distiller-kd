...
class MD5Analyzer:
    ...
    def find_md5_duplicates(self) -> bool:
        ...
        try:
            ...
            for block in self.kd_tool.blocks_data:
                ...
                if decision in [constants.DECISION_KEEP, constants.DECISION_DELETE]:
            ...
            logger.info(f"MD5 哈希计算完成。跳过标题: {skipped_titles}, 跳过代码围栏: {skipped_code_fences}, 跳过已决策: {skipped_decided}")

            # 找出重复的组
            duplicates_found = False
            self.md5_duplicates.clear() # 清空之前的结果

            logger.info(f"哈希计算完成，共有 {len(hash_groups)} 个唯一的哈希组（已排除标题等）。")
            duplicate_group_count = 0
            for md5_hash, blocks in hash_groups.items():
                if len(blocks) > 1:
                    duplicate_group_count += 1
                    logger.info(f"找到第 {duplicate_group_count} 组精确重复内容，哈希值: {md5_hash}, 块数: {len(blocks)}")
                    ...
                    try:
                        ...
                        if self.kd_tool.block_decisions.get(first_key) == constants.DECISION_UNDECIDED:
                    ...
                    for block in blocks[1:]:
                        try:
                            ...
                            if self.kd_tool.block_decisions.get(key) == constants.DECISION_UNDECIDED:
            ...
            if duplicates_found:
                logger.info(f"MD5分析完成，共找到 {len(self.md5_duplicates)} 组精确重复内容 (已排除标题)。")
            else:
                logger.info("MD5分析完成，未找到精确重复内容 (已排除标题)。")
        ...
        for i, group in enumerate(self.md5_duplicates, 1):
            ...
            for j, block in enumerate(group, 1):
                try:
                    ...
                    print(f"  决策: {decision}")
                    print(f"  原始文本预览:")
                    print(f"    {display_block_preview(block.original_text)}")
                except Exception as e:
    ...
    def review_md5_duplicates_interactive(self) -> None:
        ...
        try:
            if not self.md5_duplicates:
                logger.info("没有找到 MD5 重复内容可供审查 (已排除标题)。")
                print("\n[*] 没有找到 MD5 重复内容可供审查 (已排除标题)。")
                return
            ...
            try:
                ...
                for i, block in enumerate(group):
                    ...
                    if i == block_idx_to_keep:
                        ...
                        # 仅当块未被决策时才设置，避免覆盖其他手动决策？
                        # 或者 'a' 命令强制覆盖同组其他块？目前是强制覆盖。
                        self.kd_tool.block_decisions[key] = constants.DECISION_DELETE
