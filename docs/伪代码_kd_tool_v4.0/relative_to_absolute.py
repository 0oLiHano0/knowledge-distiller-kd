...
class RelImportRewriter(ast.NodeTransformer):
    ...
    def __init__(self, module_path: str, root_pkg: str):
        super().__init__()
        self.module_path_parts = module_path.split(".")  # 当前文件所在包路径
    ...
    def visit_ImportFrom(self, node: ast.ImportFrom):  # type: ignore[override]
        if node.level and node.level > 0:
            ...
            base = ".".join([self.root_pkg, *ancestor_parts]) if ancestor_parts else self.root_pkg
            new_module = f"{base}.{node.module}" if node.module else base

            # 构造新的 ImportFrom，level 设为 0（绝对导入）
            return ast.ImportFrom(module=new_module, names=node.names, level=0, lineno=node.lineno, col_offset=node.col_offset)
        return node


def absolute_module_path(file_path: pathlib.Path, root_dir: pathlib.Path) -> str:
    """ /root/pkg/sub/mod.py -> sub.mod """
    rel = file_path.relative_to(root_dir).with_suffix("")  # 去掉 .py
    return ".".join(rel.parts)


def process_file(py_file: pathlib.Path, root_dir: pathlib.Path, root_pkg: str, dry_run: bool = False) -> bool:
