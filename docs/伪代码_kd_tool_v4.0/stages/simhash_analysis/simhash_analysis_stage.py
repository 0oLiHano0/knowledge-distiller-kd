# kd_tool/stages/simhash_analysis/simhash_analysis_stage.py (v4.6 - Schema 路径与 task_id 更新版)
# -*- coding: utf-8 -*-
"""
=================================================
...
from .errors import SimHashAnalysisError, SimHashCalculationError, SimHashComparisonError # SimHashComparisonError 已在原文件中
...
class SimHashAnalysisStage(StageInterface): #
    ...
    def process(self, context: PipelineContextDTO) -> PipelineContextDTO: #
        ...
        **[指令]** 调用 `storage.save_content_blocks` 和 `storage.save_analysis_results` 时
        ...
        try: #
            ...
            analysis_results = self._compare_hashes_and_generate_results(all_blocks_for_comparison, run_logger, context.task_id) # 传递 task_id
            ...
            error = SimHashAnalysisError(f"SimHash 分析阶段发生未知错误: {e}", original_exception=e) #
    ...
    def _update_storage_hashes(self, blocks: List[ContentBlockDTO], logger: Logger, context: PipelineContextDTO) -> None: #
        ...
        try: #
            # [指令] StorageInterface.save_content_blocks 不再需要 task_id
            # 并且根据我们的清理，它也不需要 file_id (DTO 内部有)
            self._storage.save_content_blocks(blocks) #
            logger.debug(f"成功更新 {len(blocks)} 个 SimHash 值。") #
        except Exception as e: #
            err = SimHashAnalysisError(f"更新 SimHash 值到存储时失败: {e}", original_exception=e) #
            logger.error(err) #
    ...
    def _compare_hashes_and_generate_results(self, 
                                             blocks: List[ContentBlockDTO], 
                                             logger: Logger,
                                             task_id: UUID) -> List[AnalysisResultDTO]: # # 接收 task_id
        results = [] #
        ...
        if not block_hashes: return [] # 如果没有有效的哈希值，则不比较
        ...
        try: #
            ...
            # [指令] AnalysisResultDTO 已移除 task_id 字段，不需要在此处传递 task_id 来创建 DTO
            for b1_id, b2_id, distance in similar_pairs: #
             ...
             err = SimHashComparisonError(f"比较 SimHash 时发生错误: {e}", original_exception=e) # SimHashComparisonError 需要 original_exception
             logger.error(err) #
             # 可以在此处将 err 添加到 context.errors，如果 StageExecutionError 策略允许继续
    ...
    def _save_analysis_results(self, 
        ...
        try: #
            ...
            err = SimHashAnalysisError(f"保存 SimHash 分析结果到存储时失败: {e}", original_exception=e) #
            logger.error(err) #
            context.add_error(err) #
