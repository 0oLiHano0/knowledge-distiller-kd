# kd_tool/stages/semantic_analysis/settings_models.py (v4.6 - Semantic Settings 迁移版)
# -*- coding: utf-8 -*-
...
# --- Python 标准库及第三方库导入 ---
from typing import Optional, Literal # [指令] 导入所需类型

# --- Pydantic 导入 ---
from pydantic import BaseModel, Field, PositiveInt # [指令] 导入 Pydantic 核心


# ==============================================================================
# 语义分析阶段配置 (SemanticAnalysisStageSettings)
# ==============================================================================
# [架构师说明]: SemanticAnalysisStageSettings 定义了 P07 语义分析阶段的行为。
...
class SemanticAnalysisStageSettings(BaseModel):
    ...
    **规范**: 定义语义分析模型、阈值和执行参数。
    ...
    enabled: bool = Field(
        default=True,
        description="是否启用 P07 - 语义分析阶段 (用于语义去重)。"
    )
    ...
    device: Optional[str] = Field(
        default=None, # None 通常会让底层库自动选择 (如 'cuda' if available else 'cpu')
        description="""
