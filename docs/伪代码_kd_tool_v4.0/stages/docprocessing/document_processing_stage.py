# kd_tool/stages/docprocessing/document_processing_stage.py (v4.6 - Schema 路径与 task_id 更新版)
# -*- coding: utf-8 -*-
...
# --- 核心模块导入 ---
from ....core.interfaces import StageInterface, StorageInterface # 假设 Stage 可能需要 Storage 签名一致性
from ....core.dtos import PipelineContextDTO                     # <-- [指令] 已更新
from ....schemas.dtos import ContentBlockDTO, FileRecordDTO      # <-- [指令] 已更新 (来自中央 schemas, 已移除 task_id)
...
class DocumentProcessingStage(StageInterface):
    ...
    def __init__(
        self,
        logger: Logger,
        storage: StorageInterface, # <-- [指令] 确认类型为 StorageInterface (即使未使用)
    ...
    def process(self, context: PipelineContextDTO) -> PipelineContextDTO: #
        # ... (方法内部逻辑保持不变, 但要注意 ContentBlockDTO 的创建不再需要 task_id) ...
        # 在 _convert_raw_elements_to_dtos 方法中，创建 ContentBlockDTO 时，
        # 不再需要 task_id=context.task_id (因为 DTO 定义已移除该字段)
        task_id = context.get_task_id_str() # # 获取 task_id 字符串供日志使用
        ...
        for file_record in files_to_process: #
            ...
            try: #
                ...
                if not raw_elements: #
                    ...
                    # context.file_records[file_id].processing_status = ProcessingStatus.PROCESSED_EMPTY # 旧状态，可能需要更新为更合适的
                    file_record.processing_status = ProcessingStatus.BLOCK_EXTRACTION_FAILED # 或者一个更合适的状态
                    file_record.metadata['p03_notes'] = "No elements extracted by parser." #
                    file_record.processing_history.append({ #
                        "stage": "P03", "status": file_record.processing_status.value, #
                ...
                file_record.metadata['p03_extracted_block_count'] = current_file_block_count #
                file_record.processing_history.append({ #
                    "stage": "P03", "status": ProcessingStatus.BLOCK_EXTRACTION_COMPLETED.value, #
                ...
                if file_id in context.file_records: #
                    file_record.processing_status = ProcessingStatus.BLOCK_EXTRACTION_FAILED #
                    file_record.metadata['p03_error'] = e.to_dict() #
                    file_record.processing_history.append({ #
                        "stage": "P03", "status": ProcessingStatus.BLOCK_EXTRACTION_FAILED.value, #
        ...
        **[指令]** 创建 `ContentBlockDTO` 时 **严禁** 包含 `task_id` 字段。
        ...
        for index, raw_element in enumerate(raw_elements): #
            ...
            try: #
                ...
                # [指令] BlockType 映射逻辑，与原伪代码一致
                block_type_mapped = BlockType.UNCATEGORIZED # 更改默认值为 UNKNOWN -> UNCATEGORIZED
                if "Title" in raw_element_type_str or "Heading" in raw_element_type_str: #
                    block_type_mapped = BlockType.TITLE # 更改为 TITLE
                ...
                dto = ContentBlockDTO( #
                    # block_id 由 DTO default_factory 生成
                    file_id=file_id, #
                    text_content=element_text, #
                    analysis_text=element_text, #
                    block_type=block_type_mapped, #
                    order_in_document=index,    #
                    page_number=raw_element_metadata.get("page_number"), #
                    # task_id 字段已从 ContentBlockDTO 中移除
                    metadata={ #
