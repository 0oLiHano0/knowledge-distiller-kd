# kd_tool/schemas/dtos.py (v4.6 - task_id 重构版)
# -*- coding: utf-8 -*-
...
#               根据 v4.6 架构决策，它们不再包含 task_id 字段。
...
class FileRecordDTO(BaseModel):
    ...
    **规范**: 这是文件在系统中的唯一表示，贯穿整个处理流程。
    ...
    processing_status: ProcessingStatus = Field(
        ...
        description="文件的处理状态。**规范**: 使用 `ProcessingStatus` 枚举。"
    )
    ...
    metadata: Dict[str, Any] = Field(
        ...
        description="与文件相关的其他元数据。**规范**: 用于存储扩展信息。"
    )
    # task_id: UUID = Field(...) # <-- [架构指令 v4.6] 必须 移除此字段

    @field_validator('last_modified_at', 'registered_at', mode='before', always=True)
    ...
    class Config:
        ...
        json_encoders = {
            ...
            Path: str,
            # UUID: str # <-- [架构指令 v4.6] 不再需要，因为 task_id 移除了
        }
...
class ContentBlockDTO(BaseModel):
    ...
    **规范**: 这是进行内容分析（MD5, SimHash, Semantic）的基本单元。
    ...
    analysis_text: Optional[str] = Field(
        default=None,
        description="用于分析的标准化文本内容。**规范**: 如为 None，分析时默认使用 `text_content`。"
    )
    ...
    simhash_value: Optional[str] = Field(
        ...
        description="`analysis_text` 的 SimHash 指纹 (64位或128位)。"
    )
    metadata: Dict[str, Any] = Field(
        default_factory=dict,
        description="与内容块相关的其他元数据。"
    )
    ...
    def set_analysis_text_if_none(cls, data: Any) -> Any:
        ...
        if isinstance(data, cls): # Pydantic v2, data is the model instance
    ...
    class Config:
        extra = 'forbid'
        arbitrary_types_allowed = True
        # json_encoders = { UUID: str } # <-- [架构指令 v4.6] 不再需要
...
class AnalysisResultDTO(BaseModel):
    ...
    **规范**: 这是所有分析阶段 (MD5, SimHash, Semantic) 的标准输出格式。
    ...
    def set_pair_analysis_id_on_validation(cls, values: Any) -> Any:
        ...
        # For Pydantic v2, values is a dict
    ...
    def check_simhash_details(cls, data: Any) -> Any:
        ...
        # For Pydantic v2, data is the model instance
