...
| 迭代      | 里程碑                      | TDD 步骤                                                                                                                                                                                             | 负责人        |
| ------- | ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- |
| **Iter.0** | **BlockMerger 核心改进** | 1. 编写 `TestBlockMergerAdvanced` (Tech Lead 指导): • `test_merge_nested_code_blocks` (红) • `test_merge_mixed_language_scenarios` (红) • `test_edge_cases_empty_or_large_blocks` (红) <br> 2. 改进 `processing/block_merger.py` 通过新增测试 (绿) <br> 3. 重构 `BlockMerger` 提高可读性与可维护性 (重构) | Coding AI  |
