...
| **Iter.I** | **CzkawkaAdapter** | - [ ] I.A: 确认并规范测试文件位置与结构<br>    - [ ] I.A.1 将测试文件移动到 `tests/prefilter/` (红)<br>    - [ ] I.A.2 确认测试命名和结构 (红)<br>  - [ ] I.B: CLI 路径管理与二进制捆绑<br>    - [ ] I.B.1 测试 `get_bundled_czkawka_path()` (红)<br>    - [ ] I.B.2 实现并验证路径解析逻辑 (绿)<br>    - [ ] I.B.3 测试 Adapter 构造函数路径优先级 (红)<br>  - [ ] I.C: subprocess 错误处理与日志<br>    - [ ] I.C.1 测试各种失败场景 (红)<br>    - [ ] I.C.2 增强错误捕获和日志记录 (绿)<br>    - [ ] I.C.3 重构清理日志逻辑 (重构)<br>  - [ ] I.D: 参数配置灵活性与测试<br>    - [ ] I.D.1 测试 `_build_command` 方法 (红)<br>    - [ ] I.D.2 实现参数合并逻辑 (绿)<br>    - [ ] I.D.3 文档说明 (重构)<br>  - [ ] I.E: DTO 解析健壮性与优化<br>    - [ ] I.E.1 测试边缘 JSON 情况 (红)<br>    - [ ] I.E.2 增强解析逻辑 (绿)<br>    - [ ] I.E.3 重构解析代码 (重构) | Coding AI |
...
| **Iter.III** | **SqliteStorage 核心** | - [ ] III.A: 连接管理与初始化<br>    - [ ] III.A.1 测试初始化创建数据库文件 (红)<br>    - [ ] III.A.2 实现连接和初始化逻辑 (绿)<br>    - [ ] III.A.3 测试 finalize 关闭连接 (红)<br>    - [ ] III.A.4 实现 finalize 方法 (绿)<br>  - [ ] III.B: Schema 定义及创建<br>    - [ ] III.B.1 设计 `scanned_paths`、`duplicate_file_groups`、`duplicate_file_infos` 表 (重构)<br>    - [ ] III.B.2 测试表创建 (红)<br>    - [ ] III.B.3 实现 Schema 创建 (绿)<br>  - [ ] III.C: 存储和检索重复文件组<br>    - [ ] III.C.1 测试 `save_czkawka_scan_results` (红)<br>    - [ ] III.C.2 实现 `save_czkawka_scan_results` (绿)<br>    - [ ] III.C.3 测试幂等性 (红)<br>    - [ ] III.C.4 测试 `get_duplicate_groups_by_path` (红)<br>    - [ ] III.C.5 实现 `get_duplicate_groups_by_path` (绿)<br>  - [ ] III.D: 扫描状态检查<br>    - [ ] III.D.1 测试 `is_path_scan_recent` (红)<br>    - [ ] III.D.2 实现 `is_path_scan_recent` (绿) | Coding AI |
