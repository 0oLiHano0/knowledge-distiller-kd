...
*   **任务 3.2: 提高测试覆盖率 (本地优先)** [✅已完成]
    *   **目标:** 为新增加的配置管理、依赖注入和增强型日志系统编写测试，确保代码健壮性。
    *   **内容:**
        *   ✅ 增强核心配置模块的测试覆盖率，包括环境变量加载、验证逻辑等。
        *   ✅ 为依赖注入工厂函数编写单元测试，验证各种依赖创建场景。
        *   ✅ 在工厂函数中添加对关键参数的非空检查、确保所有资源（特别是数据库连接）在异常情况下也能正确关闭。
        *   ✅ 编写或增强集成测试，覆盖从应用启动到核心流程执行（包括配置加载、依赖创建、存储交互、日志记录）的端到端场景。
    *   **产出:** ✅ 更高的测试覆盖率（特别是配置和工厂模块），更健壮的异常处理机制。
    *   **遇到的问题:** 
        *   集成测试中对环境变量的模拟和管理较为复杂
        *   依赖注入式测试需要仔细设计测试场景
        *   测试SQLAlchemy模型需要适当的隔离和模拟